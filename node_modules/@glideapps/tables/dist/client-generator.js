...
var require_ansi_regex = __commonJS({
  "node_modules/prompt-sync/node_modules/ansi-regex/index.js"(exports, module2) {
    "use strict";
...
var require_strip_ansi = __commonJS({
  "node_modules/prompt-sync/node_modules/strip-ansi/index.js"(exports, module2) {
    "use strict";
...
// src/client-generator/bin.ts
var import_fs = require("fs");

// src/client-generator/index.ts
...
function columnNamesToObjectPropertyNames(columnNames) {
  ...
  for (const name of columnNames) {
    let rename = name;
    rename = (0, import_camelCase.default)(rename.replace(/[^a-zA-Z0-9_$]/g, "_"));
    if (!isValidIdentifier(rename)) {
      rename = (0, import_camelCase.default)("the_" + rename);
    }
    let n = 1;
    let unique = rename;
...
function tableToJavaScriptColumnSchema(table) {
  ...
  const lines = columns.map(({ displayName, internalName, type }, i) => {
...
async function tableDefinition(glide, props, appValue, tables) {
  ...
  let src = `export const ${(0, import_camelCase.default)(table.name)} = `;
  if (appValue === void 0) {
    src += `glide.table({
    app: "${props.app}",`;
  } else {
...
async function appDefinition(glide, props) {
  ...
  if (tables === void 0)
    throw new Error("Could not get tables");
  const appName = (0, import_camelCase.default)(app.name);
  let src = preamble;
  src += `
...
function predicateToSQL(predicate, resolveName) {
  ...
  if ("other" in predicate) {
    ...
    const otherColumn = resolveName(other);
    if (otherColumn !== void 0) {
...
var QueryBuilder = class {
  ...
  toSQL() {
    ...
    const predicates = this._where === void 0 ? [] : [this._where, ...this._and, ...this._or];
  ...
  where(column, compare, other) {
    this._where = { column, compare, other };
    return this;
  }
  and(column, compare, other) {
    this._and.push({ column, compare, other });
    return this;
  }
  or(column, compare, other) {
    this._or.push({ column, compare, other });
    return this;
  }
};

// src/constants.ts
var defaultEndpoint = "https://api.glideapp.io/api/function";
var defaultEndpointREST = "https://functions.prod.internal.glideapps.com/api";
...
// src/Table.ts
async function mapChunks(array, chunkSize, work) {
  const results = [];
...
var Table = class {
  constructor(props, glide) {
    this.props = props;
    this.glide = glide;
    const { columns } = props;
    this.displayNameToName = Object.fromEntries(
      Object.entries(columns).map(
        ([displayName, value]) => typeof value !== "string" && typeof value.name === "string" ? [
          displayName,
          value.name
          /* internal name */
  ...
  renameIncoming(rows) {
    ...
    const rename = Object.fromEntries(
      Object.entries(columns).map(
        ([displayName, value]) => typeof value !== "string" && typeof value.name === "string" ? [value.name, displayName] : [displayName, displayName]
      )
    );
    return rows.map(
      (row) => Object.fromEntries(
  ...
  async update(rows, row) {
    ...
    await mapChunks(Object.entries(updates), MAX_MUTATIONS, async (chunk) => {
      const response = await this.glide.post("/mutateTables", {
        appID: app,
        mutations: chunk.map(([id, row2]) => ({
          kind: "set-columns-in-row",
          tableName: table,
          columnValues: this.renameOutgoing([row2])[0],
          rowID: rowID(id)
        }))
      });
      await throwError(response);
...
var App = class {
  ...
  async getTables() {
    ...
    if (result.status !== 200)
      return void 0;
    const { data: tables } = await result.json();
  ...
  async getManifest() {
    ...
    const result = await (0, import_cross_fetch.default)(manifestUrl);
    if (result.status !== 200)
      return void 0;
...
var Glide = class _Glide {
  ...
  endpoint(path = "/") {
    let base = ["/mutateTables", "/queryTables"].includes(path) ? this.props.endpoint : this.props.endpointREST;
    if (!base.includes("://")) {
      base = `https://${base}`;
    }
    return `${base}${path}`;
  ...
  api(route, r = {}) {
    ...
    const maybeClientIDObject = clientID !== void 0 ? { "X-Glide-Client-ID": clientID } : {};
    return (0, import_cross_fetch2.default)(this.endpoint(route), {
      method: "GET",
      ...r,
      headers: {
  ...
  with(props = {}) {
    return new _Glide({ ...this.props, ...props });
  }
  /**
   ...
   */
  table(props) {
    return new Table(props, this.with(props));
  }
  /**
...
async function getOutFile(glide, appID, args = process.argv) {
  let [, , , outFile] = args;
  if (outFile === void 0) {
    const apps = await glide.getApps();
    const app = apps.find((app2) => app2.id === appID);
    const defaultFilename = `${(0, import_kebabCase.default)(app.name)}.ts`;
...
async function main(args) {
  ...
  (0, import_fs.writeFileSync)(outFile, definition);
}
main(process.argv);
