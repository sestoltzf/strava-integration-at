export type ColumnStringType = "string" | "uri" | "image-uri" | "audio-uri" | "date" | "time" | "date-time" | "markdown" | "phone-number" | "email-address" | "emoji" | "duration";
...
type ColumnTypeToType<T extends ColumnType> = T extends ColumnStringType ? string : T extends "number" ? number : T extends "boolean" ? boolean : unknown;
type ColumnTypeOrSchemaEntryToType<T> = T extends ColumnType ? ColumnTypeToType<T> : T extends ColumnSchemaEntry ? ColumnTypeToType<T["type"]> : never;
export type Row<T extends ColumnSchema> = Pretty<Partial<{
    [K in keyof T]: ColumnTypeOrSchemaEntryToType<T[K]>;
}>>;
export type NullableRow<T extends ColumnSchema> = Pretty<Partial<{
